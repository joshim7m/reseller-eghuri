...
const SKIPPED_STATUSES = ['cancelled', 'returned'];
...
const eligible = (o) => !SKIPPED_STATUSES.includes(o.status);

const skippedCount = computed(
    () => props.orders.filter((o) => !eligible(o)).length,
);
...
const itemsSold = computed(() =>
    props.orders.reduce((sum, o) => sum + Number(o.quantity || 0), 0),
);

const totalPages = computed(() =>
    Math.max(1, Math.ceil(props.orders.length / PAGE_SIZE)),
);
...
const allSelected = computed(
    () =>
        ...
        props.orders.every((o) => selected.value.has(o.id)),
);
...
function toggleAll() {
    if (allSelected.value) {
        ...
        selected.value = new Set(props.orders.map((o) => o.id));
    }
}

function exportUrl(ids) {
    return route('admin.reseller-orders.export-report', {
        from: from.value,
        to: to.value,
        ...(ids.length ? { ids } : {}),
    });
...
function exportSelected() {
    const chosen = props.orders.filter((o) => selected.value.has(o.id));
    const chosenIds = chosen.filter(eligible).map((o) => o.id);

    if (!chosenIds.length) {
        showNotice(
            'No eligible selected orders to export (cancelled/returned are skipped).',
            'error',
        );
    ...
    if (chosen.length !== chosenIds.length) {
        showNotice(
            `${chosen.length - chosenIds.length} selected order(s) skipped (cancelled/returned).`,
            'info',
        );
    }

    window.location.href = exportUrl(chosenIds);
}

function exportAll() {
    const total = props.orders.length;

    if (total && skippedCount.value) {
        showNotice(
            `${skippedCount.value} of ${total} order(s) skipped from export (cancelled/returned).`,
            'info',
        );
    }
...
<template>
    <Head title="Reseller Sales Report" />
    ...
    <AdminMaster>
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                >
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Revenue (excl. cancelled/returned)
                    </p>
                    <p
                        class="mt-1 text-2xl font-bold text-gray-900 dark:text-white"
                    >
                ...
                <table class="w-full text-sm">
                    ...
                    >
                        ...
                        >
                            ...
                            <td
                                class="whitespace-nowrap px-4 py-3 text-gray-900 dark:text-white"
                            >
                                {{ order.customer_name || '—' }}
                            </td>
                            <td
                            ...
                            <td
                                class="max-w-[220px] truncate px-4 py-3 text-gray-600 dark:text-gray-300"
            ...
            >
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        ...
                        >
                            ({{ skippedCount }} cancelled/returned excluded from
                            export)
                        </span>
                    </p>
                    <div class="flex flex-wrap gap-2">
