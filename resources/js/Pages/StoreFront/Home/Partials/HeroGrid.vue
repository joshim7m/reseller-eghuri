...
const productImage = computed(() => {
    if (!randomProduct.value?.images?.length) return null;
...
<template>
    <section class="mb-16" role="region" aria-label="Hero grid">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4 md:auto-rows-[180px] lg:auto-rows-[220px]">
            ...
            >
                ...
                <div class="relative z-10 flex h-full flex-col justify-center p-6 md:p-10">
                    <span class="text-xs font-semibold uppercase tracking-widest text-white/70">Our Starting Lineup</span>
            ...
            >
                ...
                <div class="absolute bottom-0 left-0 right-0 p-5">
                    <h3 class="text-lg font-bold text-white">{{ topCategories[1].name }}</h3>
                    <span class="mt-1 inline-flex items-center gap-1 text-sm font-medium text-white/80 underline decoration-white/40 underline-offset-4 transition-colors group-hover:text-white">
                        Discover Now
                    </span>
                </div>
            </Link>
            ...
            >
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
                <div class="absolute bottom-0 left-0 right-0 p-5">
                    <h3 class="text-lg font-bold text-white">{{ topCategories[2].name }}</h3>
                    <span class="mt-1 inline-flex items-center gap-1 text-sm font-medium text-white/80 underline decoration-white/40 underline-offset-4 transition-colors group-hover:text-white">
                        Discover Now
                    </span>
                </div>
            ...
            >
                ...
                <div class="relative z-10 flex h-full flex-col justify-center p-6 md:p-10">
                    <span class="text-xs font-semibold uppercase tracking-widest text-white/50">Explore Everything</span>
                    <h2 class="mt-2 text-xl font-bold text-white md:text-2xl">Shop All Products</h2>
                    <span class="mt-4 inline-flex w-fit items-center gap-2 rounded-full border border-white/30 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-white hover:text-charcoal">
                        Browse Collection
                        <span class="material-symbols-outlined text-[16px]" aria-hidden="true">arrow_forward</span>
                    </span>
                </div>
